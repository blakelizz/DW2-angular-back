...
app.put("/produit/:id", interceptor, (requete, resultat) => {
  ...
  if (produit.nom == null || 
    ...
    connection.query(
    "SELECT * FROM materiel WHERE name_materiel = ? AND id_materiel != ?", [produit.nom, produit.id], (err, lignes) => {
      if (lignes.length > 0) {
...
app.post("/produit", interceptor,  (requete, resultat) => {
  ...
  //validation
  if (produit.nom == null || produit.nom == "" || produit.nom.length > 20 || produit.description.length > 50) {
    return resultat.sendStatus(400); //bad request
  }
