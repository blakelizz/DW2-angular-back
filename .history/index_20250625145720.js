...
app.get("/materiel/list", interceptor, (requete, resultat) => {
  ...
  connection.query("SELECT * FROM materiel", (err, lignes) => {  //requete , function callback
...
app.put("/produit/:id", interceptor, (requete, resultat) => {
  ...
  if (produit.nom == null || 
    ...
    connection.query(
    "SELECT * FROM materiel WHERE name_materiel = ? AND id_materiel != ?", [produit.nom], (err, lignes) => {
      if (lignes.length > 0) {
        return resultat.sendStatus(409);//conflict
      }
      connection.query(
        "UPDATE materiel SET name_materiel = ?, description_materiel = ? WHERE id_materiel = ?",
        [produit.nom, produit.description, requete.produit.id],
        (err, lignes) => {
          if (err) {
            console.log(err);
...
app.post("/connexion", (requete, resultat) => {
  connection.query("SELECT u.id, u.email, u.password, r.nom FROM utilisateur u JOIN role r on u.role_id = r.id 
    WHERE email = ?",
    [requete.body.email],
    (erreur, lignes) => {
      if (erreur){
