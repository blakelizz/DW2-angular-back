...
app.delete("/produit/:id", interceptor, (requete, resultat) =>{
  ...
  connection.query("SELECT * FROM proudit WHERE id = ?", [requete.params.id], (erreur, lignes) => {
    ...
    const estProprietaire = requete.user.role == "vendeur" && requete.requete.user.id == lignes[0].id_createur;

    //validation
    if(requete.user.role != "vendeur" && requete.user.id != "administrateur"){
    return resultat.sendStatus(403);
  }
  });
})
