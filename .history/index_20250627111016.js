...
app.get("/materiel/:id", interceptor,  (requete, resultat)=> {
...
app.put("/materiel/:id", interceptor, (requete, resultat) => {
  ...
  if (materiel.nom == null || 
    ...
    "SELECT * FROM materiel WHERE name_materiel = ? AND id_materiel != ?", [materiel.nom, materiel.id], (err, lignes) => {
      ...
      connection.query(
        "UPDATE materiel SET name_materiel = ?, description_materiel = ?, capacite_materiel= ? WHERE id_materiel = ?",
        [materiel.nom, materiel.description, materiel.capacite materiel.id],
        (err, lignes) => {
          if (err) {
            console.log(err);
...
app.post("/materiel", interceptor,  (requete, resultat) => {
  ...
  //validation
  if (materiel.nom == null || materiel.nom == "" || materiel.nom.length > 20 || materiel.description.length > 50) {
    return resultat.sendStatus(400); //bad request
  }

  //verification si le nom du produit existe déjà
  connection.query(
    "SELECT * FROM materiel WHERE name_materiel = ?", [materiel.nom], (err, lignes) => {
      if (lignes.length > 0) {
        return resultat.sendStatus(409);//conflict
      }
      connection.query(
        "INSERT INTO materiel (name_materiel, description_materiel, capacite_materiel, id_createur) VALUE(?,?,?,?)",
        [materiel.nom, materiel.description, materiel.capacite, requete.user.id],
        (err, lignes) => {
          if (err) {
            console.log(err);
            return resultat.sendStatus(500); //internal server error
          ...
          // resultat.json(produit).sendStatus(201);
          resultat.status(201).json(materiel) //created (fonctionne, ajout du produit)
        }
      )
    });
})
